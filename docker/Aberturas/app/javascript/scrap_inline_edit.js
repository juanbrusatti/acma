...
class ScrapInlineEditor {
  ...
  showPendingFlash() {
    if (sessionStorage.getItem("scrapUpdated") === "true") {
      sessionStorage.removeItem("scrapUpdated");
  ...
  convertToEditMode(row) {
    ...
    // Referencia
    this.createTextInputInCell(
      cells[0],
      "ref_number",
      this.originalData.ref_number,
    );
    ...
    // Grosor
    this.createThicknessSelectInCell(
      cells[2],
      this.originalData.thickness,
      this.originalData.scrap_type,
    );

    // Color
    this.createColorSelectInCell(
      cells[3],
      this.originalData.color,
      this.originalData.scrap_type,
      this.originalData.thickness,
    );
  ...
  createThicknessSelectInCell(cell, currentValue, currentType) {
    console.log("Creating thickness select in cell:", {
      currentValue,
      currentType,
    }); // Debug
    cell.innerHTML = ""; // Clear cell content
    ...
    const availableThicknesses = Object.keys(
      this.glassOptions[currentType] || {},
    );
    console.log(
      "Available thicknesses for",
      currentType,
      ":",
  ...
  createColorSelectInCell(cell, currentValue, currentType, currentThickness) {
    console.log("Creating color select in cell:", {
      currentValue,
      currentType,
      currentThickness,
    }); // Debug
    ...
    const availableColors =
      this.glassOptions[currentType]?.[currentThickness] || [];
    console.log(
  ...
  updateThicknessOptions(thicknessSelect, glassType) {
    ...
    const availableThicknesses = Object.keys(
      this.glassOptions[glassType] || {},
    );
    console.log(
      "Updating thickness options for",
      glassType,
      ":",
      availableThicknesses,
    ); // Debug
  ...
  saveEdit(row) {
    ...
    fetch(`/scraps/${this.originalData.id}`, {
      ...
      .then((data) => {
        ...
        sessionStorage.setItem("scrapUpdated", "true");
        const url = new URL(window.location.href);
        url.searchParams.set("tab", "sobrantes");
        window.location.href = url.toString();
      })
      .catch((error) => {
        console.error("Error saving scrap:", error);
        this.showMessage(
          `Error al actualizar el retazo: ${error.message}`,
          "error",
        );
      });
  }
  ...
  validateFormData(data) {
    ...
    if (!data.color || data.color === "") {
      ...
      !["INC", "STB", "GRS", "BRC", "BLS", "STG", "NTR"].includes(data.color)
    ) {
