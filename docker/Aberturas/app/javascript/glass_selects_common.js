// glass_selects_common.js
// Módulo común para manejar selects dinámicos de vidrio
// Reutilizable para glassplates, scraps, glasscuttings, etc.
...
export function updateGlassSelects(container, selectors) {
  const { typeSelector, thicknessSelector, colorSelector } = selectors;
  
  const typeSelect = container.querySelector(typeSelector);
  const thicknessSelect = container.querySelector(thicknessSelector);
  const colorSelect = container.querySelector(colorSelector);
