...
async def run_optimize(request: Request):
    ...
    try:
        ...
        if "pieces_to_cut" not in body or "stock" not in body:
            ...
            raise HTTPException(
                status_code=400, detail="Body must contain 'pieces_to_cut' and 'stock'"
            )
        print("[LOG] Datos de entrada validados")

        # Create in-memory zip for the return to Rails
        zip_buffer = io.BytesIO()

        input = json.dumps(
            {"pieces_to_cut": body["pieces_to_cut"], "stock": body["stock"]}
        )
        ...
        for pieces_to_cut, stock in optimization_inputs:
            ...
            global_result["deleted_stock"].extend(
                optimizer_result.get("deleted_stock", [])
            )
            global_result["deleted_scrap"].extend(
                optimizer_result.get("deleted_scrap", [])
            )
            global_result["final_pieces"].extend(
                optimizer_result.get("final_pieces", [])
            )
        ...
        try:
            if all_cuts_for_summary:
                print(
                    f"[LOG] Generando PDF de resumen general con {len(all_cuts_for_summary)} cortes..."
                )
                from visualize import generate_general_summary_pdf

                summary_pdf_path = generate_general_summary_pdf(
                    all_cuts_for_summary, OUTPUT_VISUALS_DIR
                )
        ...
        body.extend(
            b'Content-Disposition: attachment; filename="Plan de corte.zip"'
            + CRLF
            + CRLF
        )
        ...
        return Response(
            content=bytes(body), media_type=f"multipart/mixed; boundary={boundary}"
        )
...
def create_optimizations_objects(input_data):
    ...
    if isinstance(input_data, str):
        try:
            ...
            raise HTTPException(
                status_code=400, detail="Invalid JSON in create_optimizations_objects"
            )

    if not isinstance(input_data, dict):
        raise HTTPException(
            status_code=400, detail="Invalid payload for create_optimizations_objects"
        )
    ...
    for color, glass_type, thickness in combos:
        pcs_combo = [
            ...
            and p.get("glass_type") == glass_type
            and p.get("thickness") == thickness
        ]

        # Filter stock by the same combo
        gps_combo = [
            g
            for g in glassplates
            if g.get("color") == color
            and g.get("glass_type") == glass_type
            and g.get("thickness") == thickness
        ]
        scs_combo = [
            s
            for s in scraps
            if s.get("color") == color
            and s.get("glass_type") == glass_type
            and s.get("thickness") == thickness
        ]
...
def optimize(pieces_to_cut, stock, zip_buffer: io.BytesIO):
    ...
    optimizer_input = json.dumps({"pieces_to_cut": pieces_to_cut, "stock": stock})
    ...
    try:
        ...
        for line in reversed(lines):
            ...
            if line.startswith("{") and line.endswith(
                "}"
            ):  # encontramos el json que nos interesa
                json_line = line
                break
        if json_line:
